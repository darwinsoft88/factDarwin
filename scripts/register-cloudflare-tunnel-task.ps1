...
$taskName = "FactuDarwin Cloudflare Tunnel"
$scriptPath = "C:\app\scripts\start-cloudflare-tunnel.ps1"
$powerShell = "$env:SystemRoot\System32\WindowsPowerShell\v1.0\powershell.exe"
$action = "`"$powerShell`" -NoProfile -ExecutionPolicy Bypass -File `"$scriptPath`""

if (-not (Test-Path -LiteralPath $scriptPath)) {
  throw "No existe el script de arranque del tunel: $scriptPath"
}
...
Write-Host "El tunel Cloudflare se levantara automaticamente cuando este usuario inicie sesion en Windows."
