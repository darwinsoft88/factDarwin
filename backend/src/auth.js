...
function verifyToken(token) {
  ...
  if (parts.length !== 3) throwUnauthorized("Token invalido.");
  const [headerPart, payloadPart, signaturePart] = parts;
  const expected = hmac(`${headerPart}.${payloadPart}`);
  if (!timingSafeEqual(signaturePart, expected)) throwUnauthorized("Firma JWT invalida.");

  const payload = JSON.parse(Buffer.from(payloadPart, "base64url").toString("utf8"));
  if (!payload.exp || payload.exp < Math.floor(Date.now() / 1000)) throwUnauthorized("Sesion expirada.");
  return payload;
}
...
function requireAuth(roles = []) {
  return (req, res, next) => {
    if (!config.authRequired) {
      req.user = { id: "dev", role: "admin", name: "Desarrollo" };
      next();
      return;
    }
    ...
    try {
      ...
      if (!match) throwUnauthorized("Falta token Bearer.");
...
function authenticateUser(snapshot, email, password) {
  ...
  if (!users.length && normalizedEmail === "admin@empresa.com" && String(password || "") === "123456") {
    return {
      id: "u-admin",
      name: "Administrador",
  ...
  if (user.passwordHash === passwordHash || user.password === password) {
    return {
      id: user.id,
      name: user.name,
      email: user.email,
