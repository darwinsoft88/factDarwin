...
describe("cliente HTTP del historial documental", () => {
  ...
  it("envia el contrato, headers y cursor sin superar 100 registros", async () => {
    ...
    await getHistoricalDocumentsPage("https://api.example", "token", "android", "device", { documentScope: "002-010", cursor: "previous", limit: 500 });
  ...
  it("propaga un codigo HTTP estructurado sin borrar el fallback local", async () => {
    ...
    await expect(getHistoricalDocumentsPage("https://api.example", "token", "web", "device", { documentScope: "002-010" }))
