    ...
    try {
      ...
      if (options?.offlineFallback) {
        showInfo(
          ...
          "Se guardó como ticket interno y se facturará cuando vuelva la conexión."
        );
      } else if (internalSaleCreditBalance > 0) {
        showWarning(
          "Nota de venta a crédito",
          `Saldo pendiente: $${internalSaleCreditBalance.toFixed(2)}`
        );
      } else {
        showSuccess(
          "Nota registrada",
          `Documento: ${persistedSale.sequence}
          Total: $${persistedSale.total.toFixed(2)}`
        );
  ...
  const issue = async () => {
    ...
    try {
      ...
      if (!client || items.length === 0) {
        showWarning("Documento incompleto", "Seleccione cliente y agregue al menos un producto.");
        return;
      }
      ...
      if (currentDocumentType === "proforma") {
        ...
        if (validationErrors.length > 0) {
          ...
          setIssueNotice(message);
          showWarning("Revise antes de guardar", message);
          return;
        }
      ...
      if (validationErrors.length > 0) {
        const message = validationErrors.map((error) => `- ${error}`).join("\n");
        setIssueNotice(message);
        showWarning("Revise antes de emitir", message);
        return;
      }
      const saleId = editingSale?.id || uid();
      const editingSaleId = editingSale?.id;
      ...
      try {
        ...
        if (finalSaleResult.status !== "AUTORIZADA") {
          ...
          setIssueNotice(message);
          showWarning("Factura no autorizada", message);
        }
      } catch (error) {
        ...
        if (transientSriError) {
          showWarning(
            "Pendiente SRI",
            `${message}\n\nLa aplicación lo reintentará cuando el SRI o la conexión respondan.`
          );
        } else {
          showError(
            "No se pudo firmar",
      ...
      if (finalSale.status === "AUTORIZADA") {
        showSuccess(
          "Factura autorizada",
          [
            `Documento: ${finalSale.sequence}`,
            `Cliente: ${client.name}`,
            `Total: $${finalSale.total.toFixed(2)}`
          ].join("\n")
        );
      } else if (finalSale.status === "PENDIENTE_SRI") {
        showWarning(
          "Factura pendiente",
          "La factura quedó pendiente de autorización del SRI."
        );
      } else {
        showInfo(
          "Factura guardada",
